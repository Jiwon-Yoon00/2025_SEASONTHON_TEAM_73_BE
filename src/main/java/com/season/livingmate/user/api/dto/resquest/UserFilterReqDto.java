...
public class UserFilterReqDto {
    ...
    @Schema(description = "음주 횟수", example = "[\"ZERO\"]")
    private List<CountRange> alcoholCount; // 음주 횟수

    @Schema(description = "잠귀 민감도", example = "[\"LOW\", \"MEDIUM\"]")
    private List<SensitivityLevel> sleepLevel; // 잠귀 민감도

    @Schema(description = "반려동물", example = "[\"강아지\", \"고양이, \"물고기\"]")
    private List<String> pet;

    @Schema(description = "정리정돈 성향", example = "[\"LOW\", \"MEDIUM\"]")
