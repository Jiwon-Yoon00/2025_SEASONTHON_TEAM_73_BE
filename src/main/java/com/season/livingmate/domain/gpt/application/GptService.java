package com.season.livingmate.domain.gpt.application;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.season.livingmate.global.config.GptConfig;
...
import com.season.livingmate.domain.gpt.api.dto.request.GptReq;
import com.season.livingmate.domain.gpt.api.dto.response.GptRes;
...
public class GptService {
    ...
    public String recommendUsers(String currentUserProfile, List<String> candidateProfiles, String weights, List<String> selectedItems) {
        ...
        String prompt = String.format("""
            ...
            추천 이유 작성 규칙 (반드시 준수):
            - reasonItems에는 selectedItems의 각 요소를 **그대로** 넣으세요. (예: "smellLevel", "sleepLevel", "tidinessLevel")
            - reasonScores에는 각 item의 일치도를 **정수(0~100)** 로만 넣으세요. (예: 92, 88, 75)
            - reasonItems와 reasonScores는 **인덱스가 서로 대응**되도록 순서를 맞추세요.
    ...
    private String callGptApi(String prompt) {
        try {
            GptReq request = new GptReq(
                    gptConfig.getModel(),
                    List.of(new GptReq.Message("user", prompt))
            );
            ...
            try (Response response = httpClient.newCall(httpRequest).execute()){
                ...
                GptRes gptRes = objectMapper.readValue(responseBody, GptRes.class);

                return gptRes.choices().get(0).message().content();
            }
        } catch (IOException e){
            throw new CustomException(ErrorStatus.GPT_CALL_FAILED);
        }
