...
public class S3Manager {
    ...
    public S3UploadResult upload(MultipartFile file, S3Folder folder) {
        ...
        try{
            ...
            // 업로드 결과 반환
            return new S3UploadResult(fileUrl, fileName, file.getSize(), file.getContentType());
    ...
    // 단일 파일 업로드 (postId 포함)
    public S3UploadResult upload(MultipartFile file, S3Folder folder, Long postId) {
        // 파일 유효성 검사
        validateFile(file);
    ...
    // 여러 파일 업로드 (postId 포함)
    public List<S3UploadResult> uploadMultiple(List<MultipartFile> files, S3Folder folder, Long postId) {
        List<S3UploadResult> results = new ArrayList<>();

        for(MultipartFile file : files){
            if(file != null && !file.isEmpty()){
                S3UploadResult result = upload(file, folder, postId);
                results.add(result);
            }
        }
        return results;
    ...
    // 채팅방별 파일 업로드
    public S3UploadResult uploadChatFile(MultipartFile file, Long chatRoomId){
        validateFile(file);
