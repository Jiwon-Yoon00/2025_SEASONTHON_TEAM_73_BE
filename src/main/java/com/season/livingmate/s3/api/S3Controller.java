package com.season.livingmate.s3.api;
...
import com.season.livingmate.s3.application.S3Service;
...
public class S3Controller {
    ...
    public ResponseEntity<Response<S3UploadResult>> uploadPostImage(
            @RequestPart("file") MultipartFile file,
            @RequestParam("postId") Long postId,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        
            S3UploadResult result = s3Service.uploadPostImage(file, postId);
    ...
    public ResponseEntity<Response<List<S3UploadResult>>> uploadPostImages(
            @RequestPart("files") List<MultipartFile> files,
            @RequestParam("postId") Long postId,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        
            List<S3UploadResult> results = s3Service.uploadPostImages(files, postId);
    ...
    public ResponseEntity<Response<S3UploadResult>> uploadChatImage(
            @RequestPart("file") MultipartFile file,
            @RequestParam("chatRoomId") Long chatRoomId,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        
            S3UploadResult result = s3Service.uploadChatImage(file, chatRoomId);
            return ResponseEntity.ok(Response.success(SuccessStatus.SUCCESS, result));
    }
    ...
    public ResponseEntity<Response<S3UploadResult>> uploadProfile(
            @RequestPart("file") MultipartFile file,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        
        Long userId = userDetails.getUser().getId(); // 인증된 사용자 ID 사용
        S3UploadResult result = s3Service.uploadProfile(file, userId);
        return ResponseEntity.ok(Response.success(SuccessStatus.SUCCESS, result));
    }
    ...
    public ResponseEntity<Response<S3UploadResult>> uploadCertificate(
            @RequestPart("file") MultipartFile file,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        
        Long userId = userDetails.getUser().getId(); // 인증된 사용자 ID 사용
        S3UploadResult result = s3Service.uploadCertificate(file, userId);
        return ResponseEntity.ok(Response.success(SuccessStatus.SUCCESS, result));
    }
