...
4. [API 응답 구조](#-api-응답)
5. [디렉토리 구조](#-디렉토리-구조)
6. [Branch & Commit 전략](#-branch--commit-전략)
...
### 핵심 개념
- **성향 조사**: 13문항 설문으로 생활 습관 구체화  
- **AI 추천**: 사용자가 선택한 3가지 기준에 맞는 룸메 후보 추천  
- **유저 필터**: 13문항 중 중요 5개 선택, 집/생활 기준 필터링  
- **홈 대시보드**: Sharer(집 있음) / Joiner(집 없음) 구분, UI/UX 개선  

## 주요 기능

### 1. 성향 조사 기반 매칭
- 출퇴근 시간, 청결 등 세부 기준 구체화  
- 누락 정보 방지 및 추천 사용자 제공  

### 2. 사용자 신원 관리
- sms 문자 인증을 통한 본인인증
- 재학증명서, 재직증명서 업로드를 통해 2단계 신원 인증 강화
...
### 4. 유저 필터
- 중요 5개 질문 기반 나에게 맞는 유저 필터링  
- 집 유무, 생활 기준 등 다중 탭 필터 제공  

### 5. 홈 대시보드
- Sharer와 Joiner 구분  
- 맞춤형 추천, 사용자 관리, UI/UX 개선  
